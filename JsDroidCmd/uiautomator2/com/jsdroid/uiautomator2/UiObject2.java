...
/**
 * A {@link UiObject2} represents a UI element. Unlike {@link UiObject}, it is
 * bound to a particular view instance and can become stale if the underlying
 * view object is destroyed. As a result, it may be necessary to call
 * {@link UiDevice#findObject(BySelector)} to obtain a new {@link UiObject2}
...
public class UiObject2 implements Searchable {
	 ...
	 */
	UiObject2(UiDevice device, BySelector selector,
		...
		mDisplayMetrics = mDevice.getAutomatorBridge().getContext()
	...
	public boolean equals(Object object) {
		...
		try {
			...
			return getAccessibilityNodeInfo().equals(
					other.getAccessibilityNodeInfo());
		} catch (Exception e) {
			return false;
		}
	}
	 ...
	 * 
	 * @param condition
	 *            The {@link UiObject2Condition} to evaluate.
	 * @param timeout
	...
	public UiObject2 getParent() {
		...
		return parent != null ? new UiObject2(mDevice, mSelector, parent)
				: null;
	}
	...
	/**
	 * Returns whether there is a match for the given criteria under this
	 * object.
	 */
	public boolean hasObject(BySelector selector) {
		AccessibilityNodeInfo node = ByMatcher.findMatch(mDevice, selector,
	...
	/**
	 * Searches all elements under this object and returns the first object to
	 * match the criteria.
	 */
	...
	/**
	 * Searches all elements under this object and returns all objects that
	 * match the criteria.
	 */
	public List<UiObject2> findObjects(BySelector selector) {
		List<UiObject2> ret = new ArrayList<UiObject2>();
		for (AccessibilityNodeInfo node : ByMatcher.findMatches(mDevice,
				selector, getAccessibilityNodeInfo())) {
	...
	/**
	 * Returns the class name of the underlying {@link android.view.View}
	...
	public String getAllText() {
		...
		for (UiObject2 child : getChildren()) {
			result.append(child.getAllText());
		}
	 ...
	 *            The end point that this object should be dragged to.
	 * @param speed
	 *            The speed at which to perform this gesture in pixels per
	 *            second.
	 */
	public void drag(Point dest, int speed) {
		...
		mGestureController.performGesture(mGestures.drag(getVisibleCenter(),
				dest, speed));
	}

	/** Performs a long click on this object. */
	public void longClick() {
		mGestureController.performGesture(mGestures
				.longClick(getVisibleCenter()));
	}
	...
	public void pinchClose(float percent) {
		pinchClose(percent,
				(int) (DEFAULT_PINCH_SPEED * mDisplayMetrics.density));
	}
	...
	public void pinchClose(float percent, int speed) {
		if (percent < 0.0f || percent > 1.0f) {
			throw new IllegalArgumentException(
					"Percent must be between 0.0f and 1.0f");
		}
		if (speed < 0) {
			throw new IllegalArgumentException("Speed cannot be negative");
		}
		mGestureController.performGesture(mGestures.pinchClose(
	...
	public void pinchOpen(float percent, int speed) {
		...
		mGestureController.performGesture(mGestures.pinchOpen(
				getVisibleBoundsForGestures(), percent, speed));
	}
	...
	public void swipe(Direction direction, float percent) {
		swipe(direction, percent,
				(int) (DEFAULT_SWIPE_SPEED * mDisplayMetrics.density));
	}
	...
	public void swipe(Direction direction, float percent, int speed) {
		...
		mGestureController.performGesture(mGestures.swipeRect(bounds,
				direction, percent, speed));
	}
	 ...
	 * @param percent
	 *            The distance to scroll as a percentage of this object's
	 *            visible size.
	...
	public boolean scroll(final Direction direction, final float percent) {
		return scroll(direction, percent,
				(int) (DEFAULT_SCROLL_SPEED * mDisplayMetrics.density));
	}
	...
	public boolean scroll(Direction direction, float percent, final int speed) {
		if (percent < 0.0f) {
			throw new IllegalArgumentException(
					"Percent must be greater than 0.0f");
		}
		if (speed < 0) {
			throw new IllegalArgumentException("Speed cannot be negative");
		}
		...
		for (; percent > 0.0f; percent -= 1.0f) {
			...
			PointerGesture swipe = mGestures.swipeRect(bounds, swipeDirection,
					segment, speed).pause(250);
	...
	public boolean fling(final Direction direction) {
		return fling(direction,
				(int) (DEFAULT_FLING_SPEED * mDisplayMetrics.density));
	}
	...
	public boolean fling(final Direction direction, final int speed) {
		ViewConfiguration vc = ViewConfiguration.get(mDevice
				.getAutomatorBridge().getContext());
	...
	public void setText(String text) {
		...
		if (UiDevice.API_LEVEL_ACTUAL > Build.VERSION_CODES.KITKAT) {
			...
			if (!node
				...
				Log.w(TAG,
						"AccessibilityNodeInfo#performAction(ACTION_SET_TEXT) failed");
			}
		} else {
			CharSequence currentText = node.getText();
			if (!text.equals(currentText)) {
				// Give focus to the object. Expect this to fail if the object
				// already has focus.
				if (!node.performAction(AccessibilityNodeInfo.ACTION_FOCUS)
						&& !node.isFocused()) {
					// TODO: Decide if we should throw here
					Log.w(TAG,
							"AccessibilityNodeInfo#performAction(ACTION_FOCUS) failed");
				}
				// Select the existing text. Expect this to fail if there is no
				// existing text.
				Bundle args = new Bundle();
				args.putInt(
				...
				if (!node.performAction(
					...
					// TODO: Decide if we should throw here
					Log.w(TAG,
							"AccessibilityNodeInfo#performAction(ACTION_SET_SELECTION) failed");
				}
				// Send the delete key to clear the existing text, then send the
				// new text
				InteractionController ic = mDevice.getAutomatorBridge()
						.getInteractionController();
				ic.sendKey(KeyEvent.KEYCODE_DEL, 0);
	...
	/**
	 * Returns an up-to-date {@link AccessibilityNodeInfo} corresponding to the
	 * {@link android.view.View} that this object represents.
	 */
	private AccessibilityNodeInfo getAccessibilityNodeInfo() {
		if (mCachedNode == null) {
			throw new IllegalStateException(
					"This object has already been recycled");
		}
		mDevice.waitForIdle();
