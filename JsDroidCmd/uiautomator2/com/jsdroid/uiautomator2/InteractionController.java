...
class InteractionController {
	...
	public boolean clickAndSync(final int x, final int y, long timeout) {
		...
		return runAndWaitForEvents(clickRunnable(x, y),
...
/**
	...
	public boolean longTapAndSync(final int x, final int y, long timeout) {
		...
		return runAndWaitForEvents(longTapRunnable(x, y),
				new WaitForAnyEventPredicate(
						AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED
								| AccessibilityEvent.TYPE_VIEW_SELECTED),
				timeout) != null;
	...
	private boolean touchUp(int x, int y) {
		...
		MotionEvent event = MotionEvent.obtain(mDownTime, eventTime,
				MotionEvent.ACTION_UP, x, y, 1.0f, 1.0f, 0, 1.0f, 1.0f,
				eventId, 0);
	...
	public boolean performMultiPointerGesture(PointerCoords[]... touches) {
		...
		event = MotionEvent.obtain(downTime, SystemClock.uptimeMillis(),
				...
				1, eventId, 0, InputDevice.SOURCE_TOUCHSCREEN, 0);
		ret &= injectEventSync(event);

		for (int x = 1; x < touches.length; x++) {
			event = MotionEvent.obtain(downTime, SystemClock.uptimeMillis(),
					getPointerAction(MotionEvent.ACTION_POINTER_DOWN, x),
					x + 1, properties, pointerCoords, 0, 0, 1, 1, eventId, 0,
		...
		for (int i = 1; i < maxSteps - 1; i++) {
			...
			event = MotionEvent.obtain(downTime, SystemClock.uptimeMillis(),
					...
					pointerCoords, 0, 0, 1, 1, eventId, 0,
					InputDevice.SOURCE_TOUCHSCREEN, 0);
		...
		for (int x = 1; x < touches.length; x++) {
			event = MotionEvent.obtain(downTime, SystemClock.uptimeMillis(),
					...
					properties, pointerCoords, 0, 0, 1, 1, eventId, 0,
					InputDevice.SOURCE_TOUCHSCREEN, 0);
			ret &= injectEventSync(event);
		}
